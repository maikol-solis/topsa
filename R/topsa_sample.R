  ...
  constructHOMOLOGY <-
    function (ivar,
      ...
      if (method == "Alpha") {
        ...
        #Persistence <- alphaComplexDiag(cbind(Y, X), printProgress = FALSE)
        #OJO!!!!!!
        Persistence <- ripsDiag( cbind(Y, X),
                                 maxdimension = 1,
...
#Entradas:---------Las mismas de la funci\'on de topsa, excepto que se agrega el par\'ametro sim, numero de Y aleatorios.

#Salidas :---------Lista que contiene la media de las diferencias entre el topsa
# -----------------original y el topsa "aleatorio" y varianza de las las distancias de bottleneck entre
# -----------------el diagrama de persistencia original y diagrama de persistencia "aleatorio". El numero de
# --------------de elemento de cada lista representa la variable usada.
...
ans <- topsa(Ydat,
             ...
             method = "Alpha",
             mc.cores = 2,
             angle = 0) # topsa original

topsa_samples <- lapply(1:sim, function(i){
  Ysample <- sample(Ydat[,1],length(Ydat[,1]))
  topsa(Ysample,Xdat)

  }) # Generaci\'on de c\'alculo de topsa con Y aleatorio


resultados <- lapply(1:NCOL(Xdat), function(j){

  valores <- lapply(1:sim, function(i) {
    #j:variable, i:numero de simulacion
    list(
      Diferencias = abs(
        ans[[j]]$Geometric.Sensitivity - topsa_samples[[i]][[j]]$Geometric.Sensitivity) ,
      Distancia = bottleneck(ans[[j]]$Diagram, topsa_samples[[i]][[j]]$Diagram)
    )
  })

    data.frame(Media_dif=mean(dplyr::bind_rows(valores)$Diferencias),
               Varianzas_dis=var(dplyr::bind_rows(valores)$Distancia))

})

}

estimacion=topsa_sample(Ydat,Xdat,sim=100)
#Interpretaci\'on: Se tiene que la media de las diferencias es m\'as alta para la variable m\'as significativa(Para este ejemplo)
# La varianza de las distancias de bottleneck, no presentan un comportamiento expl\'icito que determine cual variable es m\'as significativa(Para el ejemplo)
